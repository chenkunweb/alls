//获取时间用法：getData.dateUtils.today({ymrSign:true})
//时间格式化用法：getData.dateUtils.getFormat(new Date(),1) 默认不传为2020-20-20传1为202020-20-20:20:20:20
...
const dateUtils = {
    timeFun ({
        ...
        const addZeroMonth = addZero(date.getMonth() + 1)
        const addZeroDate = addZero(date.getDate()).toString()
        const addZeroHours = addZero(date.getHours()).toString()
        const addZeroMinutes = addZero(date.getMinutes()).toString()
        const addZeroSeconds = addZero(date.getSeconds()).toString()
        ...
        const dateHaveSigna = year + dateSigns + addZeroMonth
        ...
        if (ymrSigna) return dateHaveSigna
    ...
    //定时器专用
    getFormat(dates,nums){
        if(typeof(dates)=='string'){
            if(dates.split(' ').length ==1){
                let data = dates.split('-')
                let year = data[0]
                let addZeroMonth = addZero(data[1])
                let addZeroDate = addZero(data[2])
                return year+'-'+addZeroMonth+'-'+addZeroDate
            }else{
                let data = dates.split(' ')
                let data1 = data[0].split('-')
                let year = data1[0]
                let addZeroMonth = addZero(data1[1])
                let addZeroDate = addZero(data1[2])
                let data2 = data[1].split(':')
                let addZeroHours = addZero(data2[0])
                let addZeroMinutes = addZero(data2[1])
                let addZeroSeconds = addZero(data2[2])
                return year+'-'+addZeroMonth+'-'+addZeroDate+' '+addZeroHours+':'+addZeroMinutes+':'+addZeroSeconds
                
            }
        }else{
            const dateShifts = (time) => new Date(dates.getTime() - time * DAY_TIME)
            let a = nums==5?1:0
            const year = dateShifts(a).getFullYear().toString()
            const addZeroMonth = addZero(dateShifts(a).getMonth()+1)
            const addZeroDate = addZero(dateShifts(a).getDate()).toString()
            const addZeroDates = addZero(dateShifts(a).getDate()).toString()
            const addZeroHours = addZero(dateShifts(a).getHours()).toString()
            const addZeroMinutes = addZero(dateShifts(a).getMinutes()).toString()
            const addZeroSeconds = addZero(dateShifts(a).getSeconds()).toString()
            if(nums==1){
               
                return year+'-'+addZeroMonth+'-'+addZeroDate+' '+addZeroHours+':'+addZeroMinutes+':'+addZeroSeconds
            }else{
                if(nums==5){
                    return year+'-'+addZeroMonth+'-'+addZeroDates
                }else{
                    return year+'-'+addZeroMonth+'-'+addZeroDate
                }
