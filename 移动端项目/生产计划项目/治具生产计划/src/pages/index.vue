<template>
  <div>
    ...
    :header-cell-style="changeHeaderStyle" @header-click="click" style="width: 100%">
       ...
       <el-table-column prop="riqi" :fixed="fixed['生产日期']" :width="this.util.flexColumnWidth('riqi',tableData)" label="生产日期" > </el-table-column>
      <el-table-column prop="gongdan" :fixed="fixed['订单号']" label="订单号" > </el-table-column>
      <el-table-column prop="xinghao" :fixed="fixed['治具型号']" :width="this.util.flexColumnWidth('xinghao',tableData)"  label="治具型号"> </el-table-column>
      ...
      <el-table-column prop="shenqing"  :fixed="fixed['申请人']" label="申请人"> </el-table-column>
      <el-table-column prop="gongchengshi"  :fixed="fixed['工程师']" label="工程师"> </el-table-column>
     ...
     <el-table-column prop="shenhe"  label="审核"> </el-table-column>
      ...
      <el-table-column prop="beizhus" :fixed="fixed['备注1']" label="备注"> </el-table-column>
    </el-table>
...
export default {
  data(){
    return{
      ...
      cxValue:true,
      //固定列
      fixed:{生产日期:false,工单号:false,批号:false,产品型号:false,数量:false,状态:false,备注:false,交期:false},
      //时间选择器为空显示内容
        ...
        list.forEach((item,index)=>{
          ...
          let shenhe = item.审核人==null?'未审核':item.审核人.replace(/\s+/g, "")
            ...
            this.tableData.push({
              ...
              beizhus:item.物料配套明细.replace(/\s+/g, ""),
              shenqing:item.状态.replace(/\s+/g, ""),
              gongchengshi:item.生产批号.replace(/\s+/g, ""),
              jiaoqi:jiaoqis,
              wancheng:wancheng,
              shenhe:shenhe
