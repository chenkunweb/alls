<template>
  <div>
    ...
    :header-cell-style="changeHeaderStyle" @header-click="click"  style="width: 100%">
      ...
      <el-table-column prop="duzong" :fixed="fixed['镀种']" :width="this.util.flexColumnWidth('duzong',tableData)" label="镀种"> </el-table-column>
      <el-table-column prop="shuliang" :fixed="fixed['数量']"  label="数量"> </el-table-column>
      <el-table-column prop="danwei"  :fixed="fixed['重量']" label="重量"> </el-table-column>
      ...
      <el-table-column prop="diandu" :fixed="fixed['电镀']" label="电镀"> </el-table-column>
...
export default {
  data(){
    return{
      cxValue:true,//生产日期交期切换
      height:'',//页面高度
      fixed:{生产日期:false,工单号:false,产品型号:false,镀种:false,数量:false,重量:false,电镀:false,备注:false,交期:false},//固定列
      time:getData.dateUtils.today({ymrSign:true}),//时间选择器为空显示内容
      form:{date1:getData.dateUtils.today({ymrSign:true}),},//时间
      tableData: [],//数据
      wc:'WC11',//部门代号
      name:'电镀'//部门名
    }
    ...
    methods:{
      //切换日期重新查询
      opens(){ this.onSubmit(this.form.date1) },
