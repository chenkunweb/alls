  ...
  data(){
    ...
    methods:{
      ...
      btnslink(row, column, event){
        ...
        this.$api.get('/api/TSPL',data,res=>{
          ...
          if(JSON.parse(res.data).Table==''){
            ...
            if(JSON.parse(res.data).Table[0].查*1==1){
              ...
              // let link = 'https://www.zanty.net:8108?gongdan='+gongdan+'&date='+this.form.date1+'&username='+username+'&address='+this.wc+'&userid='+userid
        ...
        // let link = 'https://www.zanty.net:8108?gongdan='+gongdan+'&date='+this.form.date1+'&username='+username+'&address='+address+'&userid='+userid
        // let link = 'http://localhost:8080?gongdan='+row.gongdan+'&date='+this.form.date1+'&username='+username+'&address='+address
        //  window.location = link
      },
      // 固定列头部颜色
