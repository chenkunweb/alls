<template>
  <div>
    ...
    <el-table-column prop="交期" :fixed="fixed['交期']"  label="交期"> </el-table-column>
  ...
  data(){
    ...
    created(){
      ...
      this.titles={
          ...
          b:{name:'样品达成率',path:'/month'},
      }
      let yesterday,time
      if(this.$store.state.dates!=undefined){
        this.time = this.$store.state.date.split(',')
        yesterday = this.$store.state.date.split(',')[0]
        time = this.$store.state.date.split(',')[1]
      }else{
        yesterday = getData.dateUtils.thisMonthFirst({ymrSign:true})
        time = getData.dateUtils.today({ymrSign:true})
         this.time = [yesterday,time]
      }
    ...
    methods:{
        ...
        this.$api.get('/api/TSPL',data,res=>{
          ...
          if(JSON.parse(res.data).Table==''){
            ...
            if(JSON.parse(res.data).Table[0].查*1==1){
              let link = 'https://www.zanty.net:8108?gongdan='+gongdan+'&date='+row.交期.replace(/\s+/g, "")+'&username='+username+'&address='+this.wc+'&userid='+userid
              // let link = 'https://www.zanty.net:8108?gongdan='+gongdan+'&date='+row.jiaoqi+'&username='+username+'&address='+this.wc+'&userid='+userid
      ...
      async onSubmit(value){
        let data = {bm:this.wc,date1:value[0],date2:value[1]}
